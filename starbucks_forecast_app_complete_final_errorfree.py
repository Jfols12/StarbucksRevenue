...
if last_forecast > last_actual:
    ...
    {high_risk_count} high-risk quarters were flagged in 2023 with an average deviation of {avg_error_2023:.2f}%. Forecasts based on CPI ({cpi_input:.2f}) and avg ticket ({avg_ticket_input:.2f}) showed persistent misalignment with reported revenue.
    ...
    Auditors should investigate these flagged quarters thoroughly, focusing on revenue recognition timing, estimates, and the validity of recorded results. Evidence of revenue recognition that outpaces business activity may warrant extended testing and disclosure consideration.
    """
if high_risk_count == 0:
    summary_text = f"""### 📌 Summary for Audit Committee
    No high-risk quarters were identified in 2023. The ARIMAX forecast using CPI ({cpi_input:.2f}) and avg ticket ({avg_ticket_input:.2f}) closely aligns with reported revenue.
    ...
    summary_text = f"""### 📌 Summary for Audit Committee
    One high-risk quarter was flagged in 2023 where actual revenue materially exceeded forecasted values. The model, based on CPI ({cpi_input:.2f}) and avg ticket ({avg_ticket_input:.2f}), generally aligns well otherwise.
